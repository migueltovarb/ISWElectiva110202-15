...
export default function AccessPointForm({ accessPointId, isEdit = false }: AccessPointFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEdit && accessPointId) {
        await accessService.updateAccessPoint?.(accessPointId, formData);
      } else {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
        ...
        <div className="sm:col-span-2">
          <label htmlFor="max_capacity" className="block text-sm font-medium text-gray-700">Capacidad máxima</label>
          <div className="mt-1">
            <input
              type="number"
              name="max_capacity"
